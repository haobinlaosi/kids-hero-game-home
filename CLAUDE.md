...
面向约4岁儿童的网页小游戏。孩子通过完成日常任务赚货币（v31 起默认"奥特曼币"，可在小屋设置中改名；v29 起 UI 字段从"积分"改为通用"钱"，v31 又升级为可自定义货币名），用来装饰小屋、购买/培养宠物、打怪兽，并在心愿商店兑换现实奖励。主题角色：乐迪（超级飞侠）和迪迦（奥特曼）。本项目为**家庭专属版**，公众号通用版在 `kids-hero-game` 仓库。

**核心闭环（v29 起）**：完成任务 → 赚币 → 照顾宠物（每日固定支出）→ 宠物助攻打怪（v31 起每只小怪击败 +15 币）→ 剩余存起来 → 心愿商店兑换现实奖励。让 4 岁孩子在游戏里理解赚钱、花钱、存钱、延迟满足。
...
- 两个角色共享货币（`this.data.points`，UI 单位文案动态读取 `this.data.settings.currencyName`，默认"奥特曼币"，家长可在小屋设置中改名），但各自拥有独立的小屋、宠物和宠物状态
...
- **货币单位文案**（v31 起）：所有显示货币单位的地方调用 `this.currencyName()`（JS 模板）或在 init/改名时通过 `_refreshCurrencyText()` 替换 `<span class="cur-name">` 内容（HTML 静态）。新增 UI 文案中带"钱"或货币单位的，都要走这个 helper，不要硬编码
...
> 说明：下表中所有"钱"指代游戏货币单位。v31 起 UI 默认显示"奥特曼币"（家长可在小屋设置中改成任意 1-6 字名称）；为保持数值表的简洁，文档继续用"钱"作为通用占位词。
...
> v31 起，孩子还可以通过打小怪赚钱（每只 +15 钱）。任务仍是主收入（占比 ~85-95%），打怪是附加收益（详见"战斗"章节经济推演）。
...
- **普通怪兽**：10 血
  ...
  - **击败奖励（v31 新增）**：每只小怪 +15 钱。设计意图：让"养宠物"成为"激活打怪赚钱"的关键 — 4 只宠物全养时打 1 只小怪净赚 5 钱（攻击 -10、奖励 +15）；不养宠物则只能用付费攻击，每只净亏 20 钱。每只小怪奖励统一在 MONSTERS[i].rewardPoints
- **BOSS**（v28+）：50 血，**每击杀小怪 25% 随机概率触发**（已有 BOSS 在场则忽略再次触发）
  - **限时 3 天**，跨天血量保留（昨天打掉 30 血今天接着打剩余 20 血）；超时 BOSS 逃跑，不扣东西
  - 奖励：钱（普通 +50 / 终极 +100，v31 起终极从 80 提升到 100）+ **专属装饰**（自动加入当前角色小屋，商店买不到）
    - 炎魔王 → 🔥 火焰王座（家具）
    - 冰封巨兽 → 🐻‍❄️ 冰雕熊（家具）
    - 暗影恶龙 → 🛡️ 龙鳞盾（墙饰）
    - 雷霆魔神 → 🔨 雷神锤（墙饰）
...
- **新增涉及货币单位的 UI 文案时，不要硬编码"钱"或"奥特曼币"**（v31 起单位可由家长在小屋设置中改名）。JS 模板字符串用 `${this.currencyName()}` 拼接；HTML 静态文本包成 `<span class="cur-name">奥特曼币</span>`，会在 init/改名时被 `_refreshCurrencyText()` 自动更新
